...
function putWalker(screen, walker) {
  ...
  const name = walker[1];
  const botomName = walker[2];
  const x = position[0];
  const y = position[1];
  const size = walker[3];
  putString(screen, name, x, y);
  putVerticalString(screen, name, x - 1, y + 1);
  drawRectangle(screen, '.', x, y + 1, x + size, y + 1 + size);
  putStringLeft(screen, botomName, x + size , y + 2 + size);
}
...
function main() {
  ...
  const walker = [[0, -1], 'SIDDHI GARG', "SAKSHI GARG",  15, [0.75, 0.25]];
