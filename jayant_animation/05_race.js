...
function putRacer(screen, walker) {
  ...
  const x = position[0];
  const y = position[1];
  putString(screen, "<|" + name + "|>", x, y + 3);
  drawRectangle(screen, ".", x - 1, y - 1 , x + 6, y + 6);
}
...
function main() {
  ...
  const racerA = [[0, 1], 'SIDD', [0.1, 0.1], [0.02, 0.02]];
  const racerB = [[0, 11], 'ADII', [0.1, 0.01], [0.03, 0.02]];
  // const racerC = [[0, 18], 'PARTI', [0.8, 0], [0, 0]];
  const racers = [racerA, racerB];

  const frames = animate(screen, racers, 20);
