...
function putParticle(screen, particle) {
  const centrePosition = particle[0];
  const symbol = particle[1];
  const radius = particle[2];
  const angle = particle[3];

  const x = centrePosition[0] + radius * Math.cos(degreesToRadians(angle));
  const y = centrePosition[1] + radius * Math.sin(degreesToRadians(angle));

  putString(screen, 'C', centrePosition[0], centrePosition[1]);
  putString(screen, symbol, x, y);
  // putString(screen, ".", x, y);
  drawRectangle(screen, "*", centrePosition[0] - centrePosition[0] , centrePosition[1] - centrePosition[1] + 1, centrePosition[0] * 2 - 1, centrePosition[1] * 2 - 1);
}
...
function main() {
  ...
  const particle = [[WIDTH / 2, HEIGHT / 2], 'D', 15, 0, 0.1];
  const frames = animate(screen, particle, 500);
